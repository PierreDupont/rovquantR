...
source("Temp/CM/functions/Nimble/dbin_LESS_Cached_MultipleCovResponse.R")
...
if(!is.null(HABITAT$countries)){
  studyArea <- COUNTRIES[COUNTRIES$ISO %in% HABITAT$countries, ]

  ## CREATE A POLYGON OF THE ACTUAL HABITAT POLYGON CONSIDERED (different from buffered.habitat.poly)
  myBufferedArea <- st_buffer(st_as_sf(studyArea) ,dist = HABITAT$habBuffer)
  myBufferedArea$id <- 1
...
studyArea$id <- 1
studyArea <- studyArea %>% group_by(id) %>% summarize()
...
DNA <- read.csv(paste(dir.dropbox, "/DATA/RovbaseData/ROVBASE DOWNLOAD 20241023/dna_wolverines.csv", sep=""), fileEncoding="latin1")## NGS data from RovBase#[CM update to 20190627]
DEAD <- read.csv(paste(dir.dropbox, "/DATA/RovbaseData/ROVBASE DOWNLOAD 20241023/dead_carnivores.csv",sep=""), fileEncoding="latin1") ## Dead Recoveries from RovBase#[CM update to 20190627]
SUSPECT_NGS_SAMPLES <- read.csv(paste(dir.dropbox, "/DATA/RovbaseData/ROVBASE DOWNLOAD 20241023/Remove ngs samples list wolverine 2024.csv", sep = ""), fileEncoding="latin1") ## DNA samples to be removed from Henrik
SUSPECT_DeadRecoSAMPLES <- read.csv(paste(dir.dropbox, "/DATA/RovbaseData/ROVBASE DOWNLOAD 20241023/Remove dead recoveries list wolverine 2024.csv", sep = ""), fileEncoding="latin1") ## DNA samples to be removed from Henrik
HairTrapSamples <- read_xlsx(paste(dir.dropbox, "/DATA/RovbaseData/ROVBASE DOWNLOAD 20241023/hairtrapsNB2024.xlsx", sep = ""))#, fileEncoding="latin1") ## DNA samples to be removed from Henrik
DEN <- read.csv(paste(dir.dropbox, "/DATA/RovbaseData/ROVBASE DOWNLOAD 20241023/DEN_COUNTS_2009_2024_fromHB.csv", sep = ""), fileEncoding="latin1")
...
TRACKS_SINGLE <- read_sf(paste(dir.dropbox, "/DATA/RovbaseData/TRACK DATA FROM BOUVET 20240830/XX_eksport_rovquant_aktivitetslogg_alle_spor_multilinestring_20240829_dateSfAll.shp", sep = ""))
TRACKS_MULTI <- read_sf(paste(dir.dropbox, "/DATA/RovbaseData/TRACK DATA FROM BOUVET 20240830/XX_eksport_rovquant_aktivitetslogg_alle_spor_linestring_20240829_dateSfAll.shp", sep = ""))
...
# save(myFilteredData.sp, file = file.path(WD, modelName,
#                                          paste("_myFilteredData.sp", ".RData", sep = "")))
load(file.path(WD, modelName,paste("_myFilteredData.sp", ".RData", sep = "")))
...
### ==== 2. GENERATE HABITAT ====
### ====    2.1 REDUCE THE AREA OF THE STATE-SPACE BASED ON DETECTIONS [CM] ====
...
studyArea <- st_intersection(myBufferedArea, studyArea)#[studyArea$ISO %in% c("SWE","NOR"), ])
...
habitat <- MakeHabitatFromRastersf( poly = studyArea
                                    ,
...
# proj4string(habitat$buffered.habitat.poly) <- CRS(proj4string(habitat$habitat.sp))
...
habIDCells.mx[] <- 0
scaledHabGridCenters <- scaleCoordsToHabitatGrid(coordsData = habitat$habitat.xy,
...
## PLOT CHECK
pdf(file=file.path(WD, modelName, paste(modelName,"Prop id deteced_Time available.pdf",sep="")))
...
# distDestsCounties <- gDistance(detectors$main.detector.sp, COUNTIESAroundNorrbotten,byid = T)
...
DetectorIndexLESS <- GetDetectorIndexLESS( habitat.mx = habitat$habitat.mx,
...
save(habitat.list, detectors, COUNTRIES, studyArea.poly, COMMUNES,COUNTIES_AGGREGATEDSubset,
...
modelCode1 <- nimbleCode({
  ...
  for(c in 1:n.countries){
    p01Oth[c] ~ dunif(0,1)
    p0Oth[c] <- p01Oth[c] *countyToggleOth[c]## toggle counties
  }#c  
  ...
  for(i in 1:n.individuals){
    ...
    y.aliveOth[i,1:nMaxDetectorsOth] ~ dbin_LESS_Cached_MultipleCovResponse(  sxy = sxy[i,1:2],
                                                                              sigma = sigma,
                                                                              nbDetectionsOth[i],
...
for(ch in 1:4){
  ...
  for(t in 1:nYears){   
    ...
    load(file.path(WD, modelName,
                   paste(modelName,"Chain",ch, ".RData", sep = "")))
...
### ====    8.2. CHECK INITIAL LOG-LIKELIHOODS ====
cmodel$calculate()  
...
if(cmodel$calculate("y.alive") == -Inf){
  ...
  if(dim(probs)[1] < 11){
    for(d in 1:dim(probs)[1]){
      ...
      ## PLOT HABITAT SET-UP
      plot(habitat$habitat.r)
      plot(habitat$buffered.habitat.poly, add = TRUE)
      plot(studyArea, add = TRUE)
      plot(detectors$main.detector.sp, cex = 0.3, col = "gray20", pch = 16, add = TRUE)
      
      ## PLOT SIMULATED ACs & DETECTIONS
      plot(detectors$main.detector.sp[which(y.alive[i, ,t]>0), ], pch = 16, col = "navyblue", add = TRUE)
      ...
      points(detectors$main.detector.sp[detIDs,1], detectors$main.detector.sp[detIDs,2])
      
      ## PLOT DETECTIONS OUTSIDE THE ALLOWED DETECTORS
      outDets <- detectionsID[which(!detectionsID[1:detectionsNum] %in% detIDs)]
      points(detectors$main.detector.sp[outDets,1], detectors$main.detector.sp[outDets,2], pch = 16, col = "red")
    }#i
...
if(is.na(cmodel$calculate("y.alive"))){
  ...
  if(dim(probs)[1] < 11){
    ...
    for(d in sample(dim(probs)[1], 10)){
      ...
      dbin_LESS_Cached_OneCov( x = cmodel$y.alive[i, ,t]
                               ,
                               sxy = cmodel$sxy[i, ,t]
                               ,
                               sigma = cmodel$sigma
...
### ====    8.3. RESUME MODEL CONFIGURATION ====
...
outDirectories <- list.files(file.path(WD, modelName))[grep(paste("NimbleOutFOR",modelName,sep=""), list.files(file.path(WD, modelName)))]
...
#   
#   nimOutput[[i]][,"sigma"] <-  nimOutput[[i]][,"sigma"]*habitat$resolution
#   nimOutput[[i]][,"lambda"] <-  nimOutput[[i]][,"lambda"]*habitat$resolution
# }
...
{#doall[CM]
  ...
  pdf(file = file.path( WD,
                        modelName,paste(modelName,".pdf",sep="")
  ))
  ...
  ## PLOT STUDY AREA
  par(mar = c(0,0,0,0))
  #plot(habitat$buffered.habitat.poly,  col = rgb(34/250, 139/250, 34/250, alpha = 0))
  plot(st_geometry(GLOBALMAP), col = "gray80")#, add = TRUE)
  plot(st_geometry(habitat$habitat.poly), col = rgb(34/250, 139/250, 34/250, alpha = 0.5), add=T)
  ...
  load(file.path(WD, modelName,
                 paste(modelName,"Chain",1, ".RData", sep = "")))
  ...
  for(c in 1:(dim(myResults$sims.list$p0Oth)[2])){
    plot(st_geometry(studyArea))
    if(c %in% 3){ 
      plot(st_geometry(detectors$main.detector.sp[detCounties %in% 1,]), add=T, col="red")
  ...
  for(t in 1:dim(myResults$sims.list$sigma)[2]){
    plot.violins(list(myResults$sims.list$sigma[ ,t]*habitat$resolution),
                 x = t,
                 at = t,
                 violin.width = 0.2,
                 col = myCol,
...
for(t in 1:nYears){
  ...
  for(i in 1: length(nimOutput)){
    ...
    #nimOutput[[i]][,"sigma[2]"] <-  nimOutput[[i]][,"sigma[2]"]*habitat.list$resolution
    nimOutput[[i]][,"sigma"] <-  nimOutput[[i]][,"sigma"]*habitat$resolution
...
{#doall[CM]
  ...
  ### ==== 2. PLOT PARAMETERS ESTIMATES ====
  pdf(file = file.path( WD,
                        modelName,paste(modelName,"Snap.pdf",sep="")
  ))
...
gc()
# ORIGIN DENSITY
habbR <- habitat$habitat.r#raster::aggregate(habitat.list$habitat.r,fact=2)
habbR <- raster::disaggregate(habitat$habitat.r, fact=1)
habbR[habbR>0] <- 1
habbdens <- habbR
plot(habbdens)
...
pdf(file=file.path(WD, modelName, paste(modelName,"_DENSITY.pdf",sep="")))

for(t in 1: dim(sxyScaled$data.scaled.xy)[4]){
  habbdens[] <- Densi[[t]]$PosteriorsCellsMean
  
  plot(habbdens, breaks=cuts, col = col,legend=FALSE, main=years[t]) #p
  plot(studyArea.poly,add=T, border=grey(0.5))
...
habbR[this.r==1] <- 1
# 
# habbR.sp <- SpatialPoints(coordinates(habbR),proj4string = CRS(proj4string(habitat.list$habitat.r)))
# spIN <- sp::over(habbR.sp, SWE)
# habbR[!is.na(spIN[])] <- 2
plot(habbR)
##remove buffer from habitat
e <- extent(studyArea.poly)
e.sp <- as(e, 'SpatialPolygons')  
habitat.r <- habitat$habitat.r
habitat.r[habitat.r!=1] <- NA
habbR <- mask(habbR, e.sp)
# habbR.sp <- SpatialPoints(coordinates(habbR),proj4string = CRS(proj4string(habitat.list$habitat.poly)))
# spOUTBUFF <-sp::over(habbR.sp, habitat.r.wobuffer)
# spOUTBUFF <- sp::over(habbR.sp, habitat.list$habitat.poly)
...
habbRCounties <- habitat$habitat.r#raster::aggregate(habitat.list$habitat.r,fact=2)
plot(habitat$habitat.r)
...
# habbRCounties.sp <- SpatialPoints(coordinates(habbRCounties),proj4string = CRS(proj4string(habitat.list$habitat.r)))
...
# habbRCounties.sp <- SpatialPoints(coordinates(habbRCounties),proj4string = CRS(proj4string(habitat.list$habitat.poly)))
...
## TOTAL 
{#doall
  pdf(file=file.path(WD, modelName, paste(modelName,"_NRegions.pdf",sep="")))
...
#    DENSITY[[t]] <- GetPopDensityFromPolygon(habPolygon = studyArea,
#                                             numDiv.x = 10,
#                                             numDiv.y = 20,
#                                             habResolution = 20000,
...
# }#t
# 
# save(DENSITY, file = file.path( WD,
#                                 modelName,
...
for(t in 1:nYears){
  ...
  plot(DENSITY[[t]]$mean.Density, breaks = cuts, col = pal,
       ...
       legend = F, axes = F)  
  plot(studyArea, add = T, border = grey(0.5))
  plot(habitat$buffered.habitat.poly, border = grey(0.5), add = TRUE)
  plot(DENSITY[[t]]$mean.Density, legend.only = T, col = pal,
       legend.width = 1, legend.shrink = 1,
       axis.args = list( at = round(seq(0, max.cut, length.out = 11), digits = 2),
                         labels = round(seq(0, max.cut, length.out = 11), digits = 2), 
                         cex.axis = 0.6),
       legend.args = list( text = paste("Density (ind.",(OUTPUT$mapResolution/1000)^2,"km-2)",sep=""),
...
## TRY EstimateN_v3 (CM)
gc()
habbR <- raster::disaggregate(habitat$habitat.r, fact = 2)
habbR[habbR > 0] <- 1
plot(habbR)
