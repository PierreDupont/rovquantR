...
##-- Load density
load(file.path( working.dir, "data", paste0("Density_", engSpecies,"_", date, ".RData")))

##-- Load results tables
N_LastYear <- read.csv(file.path( working.dir, "tables/Abundance_LastYearPerSex.csv"))
N_All <- read.csv(file.path( working.dir, "tables/Abundance_AllYears.csv"))
```
...
```{r, density lastYear, fig.align = "center", fig.cap = paste0("Figure 1: ", engSpecies, " density in ", years[length(years)], ". Density was estimated with an open-population spatial-capture recapture model fitted to NGS and dead recovery data collected between ", years[1], " and ", years[length(years)],"."), echo = FALSE}
...
Within its primary range (`r detectors$n.detectors*(detectors$resolution/1000)^2` $km^{2}$), the `r engSpecies` population was likely (95% credible interval) made up of between `r DensityCountriesRegions[[n.years]]$summary["Total","95%CILow"]` and `r DensityCountriesRegions[[n.years]]$summary["Total","95%CIHigh"]` in `r years[n.years]`.
