...
kable( numDead_sexYear, 
       align = "lc",
       caption = "Number of cause-specific dead recoveries included in the OPSCR analysis. Numbers are reported by sex and year. Note that dead recovery data from the final season were not used in this analysis because survival is modeled as part of the transition from one season to the next.") %>%
  kable_styling(full_width = F)
```
...
detsPerYear_ <- apply(nimData_F$,c(1,3), function(x){
  ifelse(x[1] > 0, sum(x[2:(x[1]+1)]), 0)
})
numDeads_sexYear <- rbind(
  "Female legal" = colSums(nimData_F$y.dead.legal),
  "Female other" = colSums(nimData_F$y.dead.other),
  "Male legal" = colSums(nimData_M$y.dead.legal),
  "Male other" = colSums(nimData_FM$y.dead.other))
