...
covNames <- names(detectors$grid)[!names(detectors$grid) %in% c("Detector","id", "x","y","geometry")]
L <- length(covNames)
```
...
if(engSpecies == "wolverine"){
  ...
  ##-- Number of detections used 
  detsPerYear_F <- apply(nimData_F$y,c(1,3), function(x){
    ifelse(x[1] > 0, sum(x[x>0]), 0)
  })
  detsPerYear_F.Oth <- apply(nimData_F$y.Oth,c(1,3), function(x){
    ifelse(x[1] > 0, sum(x[x>0]), 0)
  })
  detsPerYear_M <- apply(nimData_M$y,c(1,3), function(x){
    ifelse(x[1] > 0, sum(x[x>0]), 0)
  })
  detsPerYear_M.Oth <- apply(nimData_M$y.Oth,c(1,3), function(x){
    ifelse(x[1] > 0, sum(x[x>0]), 0)
  })
  
  ##-- Number of sub-detectors "activated"
  numSamples_sexYear <- rbind(colSums(detsPerYear_F),
                              colSums(detsPerYear_M),
                              colSums(detsPerYear_F.Oth),
                              colSums(detsPerYear_M.Oth))
  if(length(dim(numSamples_sexYear)) > 1){
    numSamples_sexYear <- cbind(numSamples_sexYear,rowSums(numSamples_sexYear))
    colnames(numSamples_sexYear) <- c(years,"Total")
    numSamples_sexYear <- rbind(numSamples_sexYear,colSums(numSamples_sexYear))
    rownames(numSamples_sexYear) <- c("female structured",
                                      "male structured",
                                      "female unstructured",
                                      "male unstructured",
                                      "Total")
  ...
  ##-- Number of individuals used 
  numIds_sexYear <- rbind(colSums(detsPerYear_F>0),
                          colSums(detsPerYear_M>0),
                          colSums(detsPerYear_F.Oth>0),
                          colSums(detsPerYear_M.Oth>0))
  ##-- Total number of individuals per year
  numIds_sexTot <- colSums((detsPerYear_M + detsPerYear_M.Oth) > 0) + 
    colSums((detsPerYear_F + detsPerYear_F.Oth) > 0) 
  
  ##-- Total number of individuals per sampling
  numIds_totYear <- c(sum(rowSums(detsPerYear_F)>0),
                      sum(rowSums(detsPerYear_M)>0),
                      sum(rowSums(detsPerYear_F.Oth)>0),
                      sum(rowSums(detsPerYear_M.Oth)>0),
                      sum(rowSums(detsPerYear_F+detsPerYear_F.Oth)>0) +
                      sum(rowSums(detsPerYear_M+detsPerYear_M.Oth)>0))
  if(length(dim(numSamples_sexYear)) > 1){
    numIds_sexYear <- rbind(numIds_sexYear,numIds_sexTot)
    rownames(numIds_sexYear) <- c("female structured",
                                  "male structured",
                                  "female unstructured",
                                  "male unstructured",
                                  "Total")
        numIds_sexYear <- cbind(numIds_sexYear,numIds_totYear)
    colnames(numIds_sexYear) <- c(years,"Total")
  }
  kable( numIds_sexYear,
         align = "lc",
         caption = paste0("Annual number of individual ", species,"  detected via non-invasive genetic sampling and included in the OPSCR analysis. Numbers are reported by sex and for each year. We included only individuals associated with samples collected within the study area during the primary monitoring period between ", years[1], " and ", years[length(years)], ".")) %>%
    kable_styling(full_width = F)
