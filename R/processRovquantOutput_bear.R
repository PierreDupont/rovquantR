...
#' #' 
#' #' @examples
#' #' makeRovquantData_bear("bear", 2012:2021)
#' #' 
...
#'     
#'     ##-- Process MCMC output
#'     results_F <- ProcessCodaOutput_v3( nimOutput_F$samples,
#'                                        params.omit = c("sxy","z"))
#'     resultsSXYZ_F <- ProcessCodaOutput_v3(nimOutput_F$samples2)
#'     
...
#'     
#'     ##-- Process MCMC output
#'     results_M <- ProcessCodaOutput_v3( nimOutput_M$samples,
#'                                        params.omit = c("sxy","z"))
#'     resultsSXYZ_M <- ProcessCodaOutput_v3(nimOutput_M$samples2)
#'     
#'     ##-- RESCALE SXY TO THE ORIGINAL COORDINATE SYSTEM
...
#'   rrRegions <- mask(rrRegions, habitat$habitat.poly)
...
#'         aliveStates = 2,
#'         regionID = regionID,#densityInputRegions$regions.rgmx,
#'         display_progress = T,
#'         returnPosteriorCells = T)
#'       
#'       spaceUSEDM[[t]] <- GetSpaceUse_Orion(
#'         sx = densityInputRegions$sx[iter,IDMales,t],
#'         sy = densityInputRegions$sy[iter,IDMales,t],
#'         z = resultsSXYZ_MF$sims.list$z[iter,IDMales,t],
#'         sigma = sigma[iter,IDMales],
...
#'   rrNorway <- crop(rrNorway, habitat$habitat.r)
#'   
...
#'     proj4string(ACCropped[[t]]) <- proj4string(ACCropped[[t]]) <- st_crs(habitat$habitat.poly)
#'   }#t
#'   
#'   
#'   
