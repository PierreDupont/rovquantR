...
## RENAME THE LAYERS
names(SNOW) <- file.path(2008:2023,(2008:2023)+1, sep="_")

## SELECT SNOW DATA CORRESPONDING TO THE MONITORING PERIOD
SNOW <- SNOW[[file.path("X", years, "_", years + 1)]]
...
if(myVars$plot.check){
  ...
  for(t in 1:nYears){
    ...
    graphics::text(x = 100000, y = 7250000, labels = file.path(ID_TabTotal[names(NGS_TabTotal)=="N"], "IDs"), cex = 1.1, col = "firebrick3", font = 2)
    graphics::text(x = 820000, y = 6820000, labels = file.path(ID_TabTotal[names(NGS_TabTotal)=="S"], "IDs"), cex = 1.1, col = "navyblue", font = 2)
    ## ADD OVERALL NUMBERS
...
#load(file.path(myVars$WD, myVars$modelName,file.path("_myFilteredData.sp", ".RData")))
...
if(myVars$plot.check){
  ...
  for(t in 1:nYears){
    ...
    ## ADD OVERALL NUMBERS
    mtext(text = years[t], side = 3, line = 1, cex = 1.5, font = 2)
    mtext(text = file.path(sum(NGS_TabIn), "Dead Recoveries /", sum(ID_TabIn), "IDs IN"), side = 3, line = 0)
    # mtext(text = file.path(sum(NGS_TabTotal), "Recoveries /", sum(ID_TabTotal)-sum(ID_TabIn), "IDs OUT"), side = 3, line = -1)
  }#t
}
...
#DETECTIONS
pdf(file=file.path(myVars$WD, myVars$modelName, file.path(myVars$modelName,"_TRENDDetections.pdf")))
...
if(myVars$plot.check){
  plot(DEN.r)#,main=file.path("Sum Den-2013:2018Kernel H=", H[h], sep=""))
...
#PLOT
pdf(file = file.path(myVars$WD,"/",myVars$modelName,"/",myVars$modelName,"mapStructuredOthers",".pdf", sep="" ))
for(t in 1:nYears){
  year= years[t]
...
if(myVars$DATA$sex=="Hann"){
  ...
  save(myFilteredData.spM, myFullData.spM,
       ...
       file = file.path(myVars$WD, myVars$modelName,
                        file.path(myVars$modelName, "_NGSData", ".RData")) )
...
pdf(file = file.path( myVars$WD,
                      myVars$modelName,file.path(myVars$modelName,"realizedPhiCountry",".pdf")
))
par(mfrow = c(1,1))
...
###
pdf(file = file.path( myVars$WD,
                      myVars$modelName,file.path(myVars$modelName,"realizedPhiCounties",".pdf")
),width = 11,height=6)
# PHI
#NORWAY
par(mfrow = c(1,2))
