    ...
    samplingMonths = 1:12)
{
  ...
  data.alive <- st_drop_geometry(data.alive)
  ...
  dummyAlive <- rbind(dummy, data.alive[ ,c("Id","Year","Month")])
  ...
  if(!is.null(data.dead)){
    data.dead <- st_drop_geometry(data.dead)
    
    ##-- CREATE THE COMBINED DETECTION ARRAY OF DEAD & DUMMY RECOVERIES 
    dummyDead <- rbind(dummy, data.dead[ ,c("Id","Year","Month")])
