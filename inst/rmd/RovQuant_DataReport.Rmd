...
We can also look at the spatial distribution of these sapmles and individuals:
...
```{r, ngs data4, echo = FALSE, collapse = TRUE}
...
```{r, dead data3, echo = FALSE, collapse = TRUE}
##-- Number of samples per individual and year
numDet_idYear <- table(table(ngs$Id,ngs$Year))
numDet_idYear
```
...
```{r, dead data4, echo = FALSE, collapse = TRUE}
##-- Calculate distance between detections per id and year 
IDs <- unique(ngs$Id)
detDist <- list()
for(i in IDs){
  tmp1 <- ngs[ngs$Id %in% i, ]
...
We can do the same for the number and distribution of dead recoveries each year:
...
```{r data summary, eval =  F, fig.cap= "A) Nombre de détections par lièvre variable, par an, et par sexe, et B) distribution des distances entre deux detections du même individu au cours de la même année sur le site de Réotier. Les lignes pointillées montrent les valeurs moyennes pour chaque sexe." }
##-- Histogram of numbers of detections per individual per year
