...
habitat$grid <- makeGridFromCorners(
  lowerCoords = st_coordinates(habitat$lower.hab.sp), 
  upperCoords = st_coordinates(habitat$upper.hab.sp),
  crs = st_crs(habitat$habitat.sp))

isHab <- habitat$habitat.r[] == 1

habitat$grid <- habitat$grid[isHab, ]
```
...
```{r, hab grid, echo = F, fig.align = "center"}
mapview( list("habitat buffer" = habitat$buffered.habitat.poly),
         col.regions = "gray60",
         col = "gray10",
         hide = TRUE,
