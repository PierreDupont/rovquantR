  ...
  rename.list = NULL)
{
  ...
  detTracks[intersection$id] <- as.numeric(intersection$transect_L)
  TRACKS.r <- detectors$raster
  TRACKS.r[detectors$raster[] %in% 1] <- detTracks
  ...
  sk.r <- raster::rasterize(
    skandObs[skandObs$monitoring.season %in% years, 1],
    detectors$raster,
    fun = "count")[[1]]
  sk.r[is.na(sk.r[])] <- 0
  ...
  rb.r <- raster::rasterize(
    rovbaseObs[rovbaseObs$monitoring.season %in% years, 1],
    detectors$raster,
    fun = "count")[[1]]
  rb.r[is.na(rb.r[])] <- 0
  ##-- Set cells outside detector area to NA
  ...
  tmpHair <- myFullData.sp$alive %>% 
    dplyr::filter(hairTrap) 
  ...
  detCovs <- array(NA, c(dim(detTracks)[1], 2))
  detCovs[ ,1] <- detTracks
  detCovs[ ,2] <- detSnow
  dimnames(detCovs) <- list( "detectors" = 1:n.detectors,
                             "covariates" = c("tracks", "snow"))
  
  detCovsOth <- array(NA, c(dim(detTracks)[1], 3))
  ...
  for(thisSex in sex){
    ...
    z <- ifelse((y.sparse$detNums + y.sparseOth$detNums) > 0, 1, NA)
