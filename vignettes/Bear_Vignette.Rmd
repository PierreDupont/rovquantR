...
output: 
  html_document: 
    ...
    highlight: zenburn
vignette: >
  %\VignetteIndexEntry{Bear_Vignette}
...
Pierre Dupont^1^ ([pierre.dup\@live.fr](mailto:pierre.dup@live.fr)),
Cyril Milleret^1^,
Asunción Semper-Pascual^1^,
Richard Bischof^1^
...
This document presents the workflow of an OPSCR analysis of the Scandinavian Brown bear monitoring data available from [RovBase 3.0](https://rovbase.no/), as performed by the [RovQuant project](https://www.nmbu.no/en/projects/rovquant). 
...
We start by loading the different R packages necessary for the analysis, and in particular the 'rovquantR' package, which contains a set of custom wrapper functions to facilitate the analysis.
...
To make it easier (and more reproducible), we can use the 'makeDirectories' function. This will create a set of folders in the location specific by 'working_dir' that will later be used to store processed data, model input and output, as well as results figures and tables.
