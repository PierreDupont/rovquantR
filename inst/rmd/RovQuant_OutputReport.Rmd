...
propDet <- read.csv(file.path( working.dir, "tables/PropDetected.csv"), check.names = F, row.names = 1)

##-- Extract proportion detected for the last year
CI <- as.numeric(strsplit(sub('.*\\((.*)\\).*', '\\1', propDet["Total",n.years]), "-")[[1]])
```

Each year, a large proportion of the population is detected during annual large carnivore monitoring. Specifically, in `r years[length(years)]`, between `r round(CI[1]*100)`% and `r round(CI[2]*100)`% of the population were detected in the study area during non-invasive genetic sampling. 
...
propDet %>%
  kable( format = "html",
         ...
         caption = paste0("Table 2. Annual ", engSpecies, " proportion of individuals detected in the population. Estimates are based on activity center locations estimated by the open-population spatial capture-recapture (OPSCR) model. Credible intervals (95%) are shown in parentheses.")) %>%
  kable_classic(full_width = T, html_font = "Cambria") %>%
