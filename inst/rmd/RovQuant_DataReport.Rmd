...
# ## OPTION 1: Create a list to store the different layers objects
# map_list <- list()
# map_list[[1]] <- mapview( x = habitat$grid,
#          hide = FALSE,
#          legend = FALSE,
...
# 
# for(c in 1:length(covNames)){
#   map_list[[c+1]] <- mapview(habitat$grid, zcol = covNames[c], legend = TRUE)
# }
# names(map_list) <- c("Habitat", covNames)
# 
# 
# # Combine layers into a single map using Reduce and the + operator
...
# map_list <- list()
# for(c in 1:length(covNames)){
#   map_list[[c]] <- habitat$grid[ ,covNames[c]]
# }
# names(map_list) <- covNames
# mapview( map_list,
#          hide = F,
#          alpha.regions = 0.8,
#          map.types = c( "OpenStreetMap.DE",
