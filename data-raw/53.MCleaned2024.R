...
#    myStudyArea <- st_crop(COUNTRIES, st_bbox(extent(myVars$HABITAT$x.extent, myVars$HABITAT$y.extent)))
#    ## CREATE A POLYGON OF THE ACTUAL HABITAT POLYGON CONSIDERED (different from buffered.habitat.poly)
#    myBufferedArea <- gBuffer(spgeom = myStudyArea, width = myVars$HABITAT$habBuffer)
#    myBufferedArea <- gIntersection(myBufferedArea, GLOBALMAP)
...
if(myVars$plot.check){
 ...
 #####check region specific samples and detections
 par(mar=c(0,0,0,0))
 ...
 ## NSAMPLES
 myFullData.sp$alive$COUNTIES <- apply(st_intersects(COUNTIES_AGGREGATED, myFullData.sp$alive, sparse = FALSE), 2, 
                                       function(col) {which(col)})
 myFullData.sp$alive$counties1 <- 0
 for(i in 1:nrow(myFullData.sp$alive)){
   if(length(myFullData.sp$alive$COUNTIES[[i]])>0){
     myFullData.sp$alive$counties1[i] <- myFullData.sp$alive$COUNTIES[[i]][1]
   }else{
     myFullData.sp$alive$counties1[[i]] <- 0
   }
 }
 
 summa <- myFullData.sp$alive %>%
 ...
 for(t in 1:length(yearsss)){
   for(i in 1:length(dpt)){
     tmp <- myFullData.sp$alive[myFullData.sp$alive$counties1 %in% dpt[i] & myFullData.sp$alive$Year %in% yearsss[t],]
 ...
 colnames(bar) <- c(5,7,8)
 barplo <- barplot(bar,beside=T,ylab="Average Dets per ID")
 legend("topright",fill=c(grey(0.3),grey(0.6)),legend=c(2023,2024))
...
myHabitat <- MakeHabitatFromRastersf( poly = myStudyArea
                                         ,
...
habitat.subdetectors <- disaggregate(myHabitat$habitat.rWthBuffer,
                                     fact=res(myHabitat$habitat.r)[1]/myVars$DETECTORS$detSubResolution)


## GENERATE NGS DETECTORS BASED ON THE STUDY AREA
myDetectors <- myDetectors.dead <- MakeSearchGridsf( data = habitat.subdetectors
                                                   ,
...
for(ch in 1:4){
  ...
  for(t in 1:nYears){   
    ...
    load(file.path(myVars$WD, myVars$modelName,
                   paste(myVars$modelName,"Chain",ch, ".RData", sep = "")))
