...
subtitle : "RovBase data preparation"
output:
  html_document: 
    theme: "cerulean" 
    toc_depth: 2
...
if(sum(grep("bear", species, ignore.case = T))>0|sum(grep("bjørn", species, ignore.case = T))>0){
  engSpecies <- "bear"
  norSpecies <- "Bjørn"
}
...
# ##-- Identify user and load corresponding working directories
# if(Sys.info()['user'] == 'pidu') {
#   source("C:/My_documents/RovQuant/workingDirectories.R") ## Pierre
# } else if(Sys.info()['user'] == 'cymi') {
#   source("C:/My_documents/rovquant/analyses/Rgit/RovQuant/workingDirectories.R") ## Cyril
# } else if(Sys.info()['user'] == 'arichbi') {
#   source("C:/PROJECTS/RovQuant/workingDirectories.R") ## Richard
# } else stop('unknown user')
# # source(file.path(getwd(),"workingDirectories.R"))
# 
# ##-- Additional functions
# ##-- IDEALLY, WE WOULD NOT NEED OUR "PERSONNAL" LIBRARY OF FUNCTIONS
# ##-- (WOULD MAKE IT EASIER TO SHARE WITH EXTERNAL USERS)
...
##-- Intersect and extract country name
dead.recovery$Country_sf <- COUNTRIES$ISO[as.numeric(st_intersects(dead.recovery, COUNTRIES))]
```
...
dat.alive <- alive %>%
  group_by(Year, detected.earlier) %>%
  summarise(n = length(unique(Id)))
dat.alive$type = "NGS"
...
##-- Save maps as .png
png(filename = file.path(dir.out, 
