...
data.pairs <- INDIVIDUAL_ID %>% 
  filter(Status %in% c("Family group","Pair")) %>%
  rename(Year = "ReprodYear (May 1 year y - Apr 30 y+1)") %>%
  select(IdSimplified, Year) %>%
##-- Add detections of individuals in pairs from Linn
rbind(.,
      cbind.data.frame( "IdSimplified" = Pack_ID2023$Rovbase.ID,
                        "Year" = 2023),
      cbind.data.frame( "IdSimplified" = Pack_ID2023$Rovbase.ID,
                        "Year" = 2024),
      cbind.data.frame( "IdSimplified" = Pack_ID2023$Rovbase.ID,
                        "Year" = 2025)) %>%
##-- Join with NGS detections
inner_join(., myFullData.sp$alive, by = c(IdSimplified, Year)
      
##-- Kernel of NGS detections of individuals in pairs 
kern <- list()
habDens <- matrix(NA, nrow = numHabWindows, ncol = nYears)
for(t in 1:nYears){
  data.pairs.t <- data.pairs[ ,data.pairs$Years == years[t]]
  m.xy <- matrix(NA, nrow = nrow(data.pairs.t), ncol = 2)
  colnames(m.xy) <- c("x","y")

  IDs <- unique(data.pairs.t$IdSimplified)
  
  for(i in 1:length(IDs)){
    tmp <- data.pairs.t[data.pairs.t$IdSimplified == IDs[i], ]
  ...
  kern <- raster(estUDm2spixdf(kernelUD( as(locationsFamily[ ,"id"], "Spatial"), 
                                         h = 15000,
                                         grid = as(habitat$habitat.r, 'SpatialPixels'))))
  habDens[ ,t] <- scale(kern[habitat$habitat.r[ ] == 1])
} #t
