...
#'
#' @description
#' The \code{dbinomLocal_normalWolverine} distribution is a NIMBLE custom distribution which can be used to model and simulate
...
#'
#' The \code{dbinomLocal_normalWolverine} distribution incorporates three features to increase computation efficiency (see Turek et al., 2021 <doi.org/10.1002/ecs2.3385>  for more details):
#' \enumerate{
...
#' }
#' 
#' The \code{dbinomLocal_normalWolverine} distribution requires x- and y- detector coordinates (\emph{trapCoords}) and activity centers coordinates (\emph{s}) to be scaled to the habitat grid (\emph{habitatGrid}) using the \code{\link{scaleCoordsToHabitatGrid}} function.
#'
...
#' 
#' 
#' @name dbinomLocal_normalWolverine
#'
...
#' @param p0 Vector of baseline detection probabilities for each trap used in the half-normal detection function. When \emph{p0Traps} is used, \emph{p0} should not be provided. 
...
#' i=1
#' 
#' # OPTION 1: USING 3 OBJECTS (y, detIndices, detNums)
#' dbinomLocal_normalWolverine( x = SparseY$y[i,,1],
...
#'                              indicator = indicator)
#'                                                                 
#' # OPTION 2: USING THE SPARSE REPRESENTATION (WE DO NOT PROVIDE THE detNums AND detIndices ARGUMENTS)
#' dbinomLocal_normalWolverine( x = SparseY$yCombined[i,,1],
#'                    size = rep(1,4),
#'                    p0 = p0,
#'                    sigma= sigma, 
#'                    s=s[i,1:2],
...
#' rbinomLocal_normalWolverine(n=1,
#'                    size=rep(1,4),
#'                    p0 = p0,
#'                    sigma= sigma, 
#'                    s=s[i,1:2],
  ...
  run = function( x = double(1),
    ...
    for(r in 1:localTrapsNum[sID]){
      ...
      if(theseLocalTraps[r] == detIndices1[count]){ 
        logProb <-  logProb + dbinom( x1[count],
                                      prob = p,
                                      size = size[theseLocalTraps[r]],
                                      log = TRUE)
        count <- count + 1
      } else {
        logProb <- logProb + dbinom( 0,
                                     prob = p,
                                     size = size[theseLocalTraps[r]],
                                     log = TRUE)
      }
  ...
  run = function( n = double(0, default = 1),
    ...
    for(r in 1:localTrapsNum[sID]){
      ...
      pZero <- ilogit(logit(p0[trapCovsIntercept[theseLocalTraps[r]]]) +
                        indBeta * indCov +
                        inprod(trapBetas, trapCovs[theseLocalTraps[r], ]))
      p <- pZero * exp(alpha * d2)  # Draw the observation at detector j from a binomial distribution with probability p
