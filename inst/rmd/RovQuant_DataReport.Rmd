...
kable( NGSCountry_F, 
       align = "lc",
       caption = paste0("Annual number of female non-invasive genetic samples included in the analysis. Numbers are reported by country and year. We included only samples collected within the study area during the primary monitoring period between ", years[1], " and ", years[length(years)], ".")) %>%
...
Dead_F <- Dead_SEX[-1,c(1,which(Dead_SEX[1,  ]%in% "F"))]
kable( Dead_F, 
       align = "lc",
       caption = "Number of cause-specific female dead recoveries included in the OPSCR analysis. Numbers are reported by country, year, and mortality cause. Note that dead recovery data from the final season were not used in this analysis because survival is modeled as part of the transition from one season to the next.") %>%
  kable_styling(full_width = F)

Dead_M <- Dead_SEX[-1,c(1,which(Dead_SEX[1, ] %in% "M"))]
kable( Dead_M, 
       align = "lc",
       caption = paste0("Number of cause-specific male dead recoveries included in the OPSCR analysis. Numbers are reported by country, year, and mortality cause. Note that dead recovery data from the final season were not used in this analysis because survival is modeled as part of the transition from one season to the next.")) %>%
  kable_styling(full_width = F)
```
...
load(file.path( working.dir, "nimbleInFiles", "male",
                paste0("nimbleInput_", date, "_male_1.RData")))
nimData_M <- nimData

load(file.path( working.dir, "nimbleInFiles", "female",
                paste0("nimbleInput_", date, "_female_1.RData")))
nimData_F <- nimData
```
...
```{r, nimble data, echo = TRUE}

  ##-- Table of #of detections 
  detsPerYear_M <- apply(nimData_M$y.alive,c(1,3), function(x){
    ifelse(x[1] > 0, sum(x[2:(x[1]+1)]), 0)
  })
  numSamplesPerYear_M  <- colSums(detsPerYear_M)
  numIdsPerYear_M  <- colSums(detsPerYear_M > 0)
  
  ##-- Table of #of detections 
  detsPerYear_F <- apply(nimData_F$y.alive,c(1,3), function(x){
    ifelse(x[1] > 0, sum(x[2:(x[1]+1)]), 0)
  })
  numSamplesPerYear_F <- colSums(detsPerYear_F)
  numIdsPerYear_F <- colSums(detsPerYear_F > 0)
